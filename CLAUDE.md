...
- **Data knobs — polygon, destination types, forecast window — keep the explicit commit.** Each is a real fan-out against donated free APIs, and a live-updating calendar drag would 429 the exploring user against their own limit. The forecast window is the one data knob that also reports itself through `commitNeeded` (as `window-changed`): a calendar makes changing it a click, so a report can go stale while the panel looks settled (#166).
...
- `src/utils/calendar.ts` — the forecast window as a calendar (#166): the servable band, the `ForecastSelection` type (`now` | `days` with optional narrowed `hours`), month-grid generation, the click/drag reducers, and the conversion to the `datetime-local` pair everything downstream reads. All of it pure, because Vitest has no DOM and a `ForecastCalendar.test.tsx` would be silently uncollected — the component is wiring only. **The band's far edge is measured and zone-dependent**: `FUTURE_LIMIT_DAYS` is 15 (Open-Meteo's "16 days" counts today) and `bandEnd` walks back a further day wherever a local 23:59 lands on the next UTC date, because the request carries UTC dates. `urlState`'s `classifyWindow`/`classifyAqiCoverage` read the same band so the grid and the warnings cannot disagree
