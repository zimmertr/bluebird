...
- `src/components/ForecastCalendar.tsx` — the month grid, the Now chip, and the narrow-hours disclosure. Holds no date logic of its own (see `utils/calendar.ts`); a mouse drags a range, a finger taps twice, and the selection commits on pointerup so a drag never re-renders the panel per pointermove
...
- `src/utils/calendar.ts` — the forecast window as a calendar (#166): the servable band's constants, the `ForecastSelection` type (`now` | `days` with optional narrowed `hours`), month-grid generation, the click/drag reducers, and the conversion to the `datetime-local` pair everything downstream reads. All of it pure, because Vitest has no DOM and a `ForecastCalendar.test.tsx` would be silently uncollected — the component is wiring only
