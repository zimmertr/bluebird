...
describe('the map chrome anchors', () => {
  ...
  it('leaves no bottom offset spelled in a component', () => {
    ...
    expect(stylesSource).toContain("compact: 'top-25 touch:top-29'")
    expect(stylesSource).toContain("full: 'top-36 touch:top-42'")
    ...
    expect(LEGEND_TOP_PX).toBe(42 * 4)
...
describe('the resting height', () => {
  ...
  // the band is 190 of the 265 the stack wants, and the key's last bands
  ...
  describe.each([
    ...
    ['chart and table', { chartShown: true, tableShown: true }, 2, 190],
...
describe('the drag cap', () => {
  ...
  it('states both caps outright', () => {
    expect(DRAGGED_MAP_PX).toBe(292)
    expect(maxSheetPx(874)).toBe(582)
    expect(maxSheetPx(757)).toBe(465)
  })
