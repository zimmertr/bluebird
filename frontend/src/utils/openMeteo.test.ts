...
// This file's own text, for the same reason: a message is copy the moment it
// is thrown here, and the guard below reads every throw in one pass.
...
describe('freezing level unit', () => {
  ...
  it('says the request failed, not that Open-Meteo is unreachable', () => {
    ...
    // for any unusable body (weather.py `_freeze_to_ft`).
...
// ── Every message this file throws ─────────────────────────────────────────

describe('the messages this file throws', () => {
  ...
  it('end with the standing tail, and carry no counts', () => {
    const thrown = [...openMeteoSource.matchAll(THROWN_LITERAL)]
  ...
  it('give an unreadable reply one spelling', () => {
    ...
    expect(openMeteoSource.split(BAD_BODY_MESSAGE)).toHaveLength(2)
  })
})
