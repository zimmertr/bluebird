...
describe('the model-coverage sentence', () => {
  // These two literals are the backend's, minus the model label that neither
  // side shares: `_coverage_message` in backend/app/services/weather.py. Issue
  // #380's manifest will hold the same text and compare it here; until it
  // lands this is what holds the two languages together.
  it('is the sentence the backend composes', () => {
    expect(COVERAGE_PHRASE).toBe('has no forecast coverage for this area.')
    expect(COVERAGE_MESSAGE_TAIL).toBe(
      'has no forecast coverage for this area. Switch to a different model and try again.',
    )
  })
