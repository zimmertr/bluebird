...
const throwingSources = [openMeteoSource, aggregateSource, errorsSource].join('\n')
...
describe('freezing level unit', () => {
  ...
  it('fails instead of guessing at an unknown unit', () => {
    expect(() => rainierMetrics([2560.0], 'furlongs')).toThrow(OpenMeteoBadBody)
    expect(() =>
      weatherSeries(
    ...
    ).toThrow(OpenMeteoBadBody)
  })

  it('says the request failed, not that Open-Meteo is unreachable', () => {
    // The body arrived. Blaming the network sends the reader after a problem
    // they do not have, and the browser path is the only path since #240, so
    // this sentence is the one a user sees. It is the backend's own wording
    // for any unusable body (aggregation.py `_freeze_to_ft`).
    expect(() => rainierMetrics([2560.0], 'furlongs')).toThrow(
      'Open-Meteo request failed. Try again later.',
    )
    expect(() => rainierMetrics([2560.0], 'furlongs')).not.toThrow(OpenMeteoUnreachable)
    expect(() => rainierMetrics([2560.0], 'furlongs')).not.toThrow(/Cannot reach/)
  })

  it('fails when the column carries numbers and no unit', () => {
    expect(() => rainierMetrics([2560.0], null)).toThrow(OpenMeteoBadBody)
  })
...
describe('fetchWeather', () => {
  ...
  it('requests the level winds, the level temperatures and the freezing level alongside the surface variables', async () => {
    const fetchSpy = vi.fn(async () => jsonResponse(hourlyPayload()))
    vi.stubGlobal('fetch', fetchSpy)
    await fetchWeather(
      [{ latitude: 47.5, longitude: -121.9 }],
  ...
  it('fails the fetch when the freezing level unit is unreadable', async () => {
    ...
    ).rejects.toThrow(OpenMeteoBadBody)
  })
  ...
  it('says a miscounted response failed, and counts it only in the console', async () => {
    // The body arrived, so this is the same fault as an unreadable unit and
    // owes the same sentence. #431: the reader met "Open-Meteo returned 2
    // results for 1 locations", a developer note with no remedy in it.
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {})
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => jsonResponse([hourlyPayload(), hourlyPayload()])),
    )
    const failing = fetchWeather(
      [{ latitude: 0, longitude: 0 }],
      WINDOW.startMs,
      WINDOW.endMs,
      OPTS,
    )
    await expect(failing).rejects.toThrow(BAD_BODY_MESSAGE)
    await expect(failing).rejects.not.toThrow(/results for/)
    expect(warn).toHaveBeenCalledWith(expect.stringContaining('2 results for 1 location'))
    warn.mockRestore()
  })
...
describe('callWeight', () => {
  ...
  it('multiplies the variable count by the model count', () => {
    ...
    // the same as one model with N times the variables (issue #232). Nine
    // variables across three models is 27 series: factor 2.7.
...
// ── The model-coverage sentence ────────────────────────────────────────────

describe('the model-coverage sentence', () => {
  // The two constants are compared with the backend's `_coverage_message` in
  // `mirroredConstants.test.ts`, which reads the manifest (#380). This block
  // holds the other half: no surface spells the words for itself.
  it('is spelled in no surface that says it', () => {
...
// ── Every message the Open-Meteo modules throw ─────────────────────────────

describe('the messages the Open-Meteo modules throw', () => {
  // The browser path is the only path since #240, and `useAnalyze` puts
