...
// `?raw` gives the file's text without executing it. The layer the field draws
// into is `map/overlays/forecastGrid.ts`, which needs a map, so what it wires is
// asserted as source here and as behavior against a stub map in its own test.
// What App.tsx wires is the `app-grid-gate` check in tools/eslint/checks/app.js.
import gridLayerSource from '../map/overlays/forecastGrid.ts?raw'
...
describe('the grid opens as a field rather than as blocks', () => {
  ...
  it('changes a paint property and not a layer', () => {
    expect(gridLayerSource).toContain("style === 'smooth' ? 'linear' : 'nearest'")
    expect(gridLayerSource.match(/'raster-resampling'/g)).toHaveLength(2)
  })
})
...
describe('how the grid layer leaves the map', () => {
  it('builds the pixels it hands the source', () => {
    // `rasterImage`, which builds the pixels, lives beside the layer.
    expect(gridLayerSource).toContain('export function rasterImage(')
  })

  it('clears the field by hiding the layer', () => {
    expect(gridLayerSource).toContain(
    ...
    expect(gridLayerSource).toMatch(/id: 'forecast-grid-fill',[\s\S]{0,400}?visibility: 'none'/)
  })
})
