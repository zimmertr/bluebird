...
by row. A destination outside coverage has no data in the table's
**Wildfire (mi)** column and in the same column of a downloaded CSV (the dash
on screen, an empty cell in the file), and a checked row is never blank — it
carries a distance or the `>10` it cleared — so a missing warning is never
mistaken for a clear check, while a covered destination in the same table
keeps its real answer. The outline is
