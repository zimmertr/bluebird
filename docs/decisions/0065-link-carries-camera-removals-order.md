...
- `view=lng,lat,zoom`: four decimals of position and two of zoom. It wins over the app's opening fits to the ring, the pasted list and the pins, but not over a search or a paste the reader makes. A pan or a zoom by the reader alone makes a link; the app's own camera moves keep the camera in a link that exists and make none by themselves. The camera reaches the link's writer without React state, so a pan renders nothing.
...
- The fields a pan hands MapLibre's `moveend`: `originalEvent` for a drag, a wheel, a touch, a key, a box zoom and the zoom buttons, and `geolocateSource: true` for the geolocate button, read in the maplibre-gl 6.10 source on 2026-09-24. The app's own `fitBounds`, `jumpTo` and `flyTo` calls carry neither.
