...
The results table carries the comparison too. With models compared it grows a **Model** column, sitting directly after the identity columns and before the first metric, and every destination gets one row per model: two destinations under three models is six rows. The rank is the destination's, so the rows of one destination all read the same number, and the table can still be sorted by any column including **Model**. The downloaded CSV is the same table, so it carries the column and the rows as well. With one model selected the column is absent and every destination is one row, as it always was.
