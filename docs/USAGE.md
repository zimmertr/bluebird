...
**The temperature columns read `Temperature at elevation` over a forecast window**, and a plain `Temperature` over an archive or crossing one — where the pressure levels are not published and the number is the plain near-ground reading the unqualified header already names.
...
| Temperature at elevation · Min/Max/Avg (°F) | Temperature range and average over the window, read at the destination's own elevation. Reads `Temperature` over an archive window |
