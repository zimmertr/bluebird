...
The chart then draws one line per destination per model: three destinations under three models is nine lines. Every line is solid, and color says which of the two things a line is read by. Lines from the model that ranks wear their destination's color, the same color it wears in the table and on the map, so those read as the destinations you already know. Lines from every other model wear one color for that model, across all of its destinations at once, so you can see what a second model does to the whole picture. The chips beside the chart's metric radios are the key: a filled square in each model's color beside its name, and no square on the ranking model, because its lines carry the destinations' colors rather than one of its own. Every line in the hover box names all three things it is: rank, destination, model, as in **1. Mount Rainier (NOAA GFS)**. That is where you read the destination of a compared line. A chip marked **Blend** serves one agency's fine regional model for roughly the first two days and its coarse global model after that, so those lines change model partway along.
