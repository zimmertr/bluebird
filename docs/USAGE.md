...
Nine lines is a lot to read at once, so **Models** in the results bar puts some of them down. It sits beside **Columns**, appears whenever more than one model is on the chart, and opens the same kind of popover: one row per model, in the picker's order, each with a checkbox and a filled square in that model's line color. The model that ranks has no square, because its lines wear the destinations' colors rather than one of its own. Untick a model and its lines leave the chart; tick it back and they return. Nothing else moves: the forecasts are already bought, so this costs nothing either way, and it touches no ranking, no table, no file and no link. Unticking every box is allowed, and leaves an empty chart. A model you unselect in the picker and select again comes back showing.
...
Air quality has no comparison, so **Models** is absent on that metric: AQI comes from one model whatever forecast model ranks the field. See [Data Sources](DATA.md) for the rest of the caveats.
