...
**A window older than about two months is served from a different place**, and the far past is the one stretch of the calendar where Step 2's model does not apply: those hours come from Open-Meteo's archive, which is one recorded dataset rather than a forecast any model made, so the model control is faded out while such a window is selected. Hovering it says why, and so does a screen reader. Wind is measured 10 m above the ground there rather than adjusted to each summit, for the reason [DATA.md](DATA.md#open-meteo) gives.
...
The chart then draws one line per destination per model: three destinations under three models is nine lines. Every line is solid, and color says which of the two things a line is read by. Lines from the model that ranks wear their destination's color, the same color it wears in the table and on the map, so those read as the destinations you already know. Lines from every other model wear one color for that model, across all of its destinations at once, so you can see what a second model does to the whole picture. There is no key beside the chart: move the cursor over it, and every line in the hover box names all three things it is: rank, destination, model, as in **1. Mount Rainier (NOAA GFS)**. That is where you read which model a line came from. A model the picker marks **Blend** serves one agency's fine regional model for roughly the first two days and its coarse global model after that, so those lines change model partway along.

Nine lines is a lot to read at once, so **Models** in the results bar puts some of them down. It sits beside **Columns** and is there whenever **Columns** is. It opens the same kind of popover: one row per model you have selected in the picker, the ranking model first and the rest in the picker's order, each with a checkbox and a filled square in that model's line color. The model that ranks has no square, because its lines wear the destinations' colors rather than one of its own. A model you have just selected has a row straight away, before the Analyze that buys its lines, and the square it shows is the color those lines draw in. Model colors are taken from past the colors the charted destinations wear, so charting another destination can move one. Untick a model and its lines leave the chart; tick it back and they return, and a model unticked before an Analyze stays hidden when its lines arrive. Nothing else moves: the forecasts are bought either way, so this costs nothing, and it touches no ranking, no table, no file and no link. Unticking every box is allowed, and leaves an empty chart. A model you unselect in the picker and select again comes back showing.
