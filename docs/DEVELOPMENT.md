...
| `checks/app.js` | `app-legend-anchors`, `chrome-bottom-offsets`, `map-legend-anchors`, `layers-popover-dismiss`, `app-transport-anchor`, `app-docked-panels` | No bottom offset or bottom-anchored legend spelled in a component; the Layers popover closes on a click away and on Escape; the docked panel defaults and floors |
