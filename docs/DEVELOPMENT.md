...
| `checks/app.js` | `app-map-stage`, `map-stage`, `map-stage-memo-props`, `analysis-overlay-clock`, `map-stage-anchors`, `map-legend-anchors`, `layers-popover-dismiss`, `app-transport-anchor`, `app-docked-panels` | App renders the map column once through `MapStage` and builds nothing for it; `MapStage` runs no effect, draws the legend before the buttons, and hands `MapView` only members of its inputs; the loading card runs its own clock; no bottom offset or bottom-anchored legend spelled in a component; the Layers popover closes on a click away and on Escape; the docked panel defaults and floors |
