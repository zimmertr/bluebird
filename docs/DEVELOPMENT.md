...
| `checks/styles.js` | `style-map-column`, `style-map-wrapper`, `style-map-legend-column`, `style-map-button-column`, `style-map-layers-column`, `style-results-bar`, `style-legend-ramp`, `style-layer-rows`, `style-search-box`, `style-chart-metric`, `style-axis-item`, `style-rank-cell` | The map column, the results bar, the legend strip, the Layers rows, the search box, the chart select and the rank cell wear their roles |
