...
| `SNODAS_CACHE_TTL_S` | `3600` | How long the held SNODAS snow depth grid counts as current. One grid is published a day, so a check on this interval is usually one `HEAD` request that finds the day already in hand; it is hourly rather than daily because the day's file lands mid-morning UTC and a server started before then would otherwise serve the previous day's grid for a whole day. Past it the grid is still read, with a refresh running behind the request. |
| `SNODAS_RETRY_AFTER_FAILURE_S` | `300` | How long a failed snow depth refresh suppresses the next attempt. Longer than the overlay twins above, because the product changes once a day and nothing is lost by waiting. |
