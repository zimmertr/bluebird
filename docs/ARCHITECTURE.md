...
Bluebird is one FastAPI service that also serves the built React SPA as static files. The web app asks the server only for discovery — `POST /api/destinations`, one Overpass query — and fetches the Open-Meteo forecasts **directly from the browser**, so each visitor spends their own free-tier quota instead of the server's one shared egress IP. The browser's aggregation is a port of the backend's, pinned identical by shared test vectors (`weather_vectors.json`, checked by both suites and diffed in CI). The browser path is the app's only analysis path: #240 removed the fallback that rerouted an analysis through `POST /api/analyze/stream` on the server's shared quota, so a browser that cannot reach Open-Meteo gets an honest error instead. The analyze endpoints themselves remain, for direct API callers, in-cluster release probes, and self-hosted instances — but the production gateway does not route them from the internet (the chart's VirtualService answers them with the app's own JSON 404).
