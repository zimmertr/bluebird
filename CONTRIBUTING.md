...
- A change to the weather or air-quality aggregation changes the backend first, regenerates `backend/tests/data/weather_vectors.json`, copies it to `frontend/src/utils/weather_vectors.json`, and mirrors the change in the TypeScript port. CI fails the PR if the two copies differ.
...
- A new source file under `frontend/src` or `backend/app` gets a line in the matching layout list in `CLAUDE.md`, saying what it owns and why it is separate. CI fails the PR otherwise.
